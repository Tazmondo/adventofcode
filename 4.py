...
for passport in passports:
    valid = map(lambda a: a in passport, fields)
    if all(valid):
        goods.append(passport)

for i in goods:
    passport = i.replace("\n", " ").strip()
    termstext = passport.split(" ")
    terms = []
    for term in termstext:
    ...
    for term in terms:
        ...
        if key == "byr":
            ...
            if value.startswith("#"):
                code = value[1:]
                if len(code) == 6 and all(map(lambda a: a in "0123456789abcdef", code)):
                    continue
            valid = False
